...
const dbPath = path.resolve('C:\\Users\\xjwoz\\OneDrive\\Desktop\\_Dasom_Hackathon', 'warmth_wall.db');
const db = new Database(dbPath);

try {
  db.exec(`INSERT OR IGNORE INTO users (id, email, password_hash) VALUES ('@dasom_ai', 'dasom@test.com', '12345678')`);
} catch (e) {
  console.log(e);
}

try { db.exec('ALTER TABLE posts ADD COLUMN photo TEXT;'); } catch (err) {}
try { db.exec('ALTER TABLE posts ADD COLUMN color TEXT;'); } catch (err) {}
try { db.exec('ALTER TABLE posts ADD COLUMN category TEXT;'); } catch (err) {}

db.exec('DELETE FROM posts;');

const TODAY = '2026-08-07'; // Ensure this matches what the app considers TODAY
...
SEED_POSTS.forEach(p => {
  ...
  insertStmt.run(p.id, p.content, p.author, '@dasom_ai', p.color, p.category, createdAt);
});

console.log('DB Seeded with ' + SEED_POSTS.length + ' posts!');
