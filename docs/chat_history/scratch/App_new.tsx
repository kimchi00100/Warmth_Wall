...
function AuthScreen({ onLogin }: { onLogin: (handle: string) => void }) {
  ...
  return (
    <div style={{ width: '100%', height: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <img src={boardBg} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
      ...
      <div style={{ width: '100%', maxWidth: 360, padding: '0 26px', boxSizing: 'border-box', position: 'relative', zIndex: 1 }}>
        ...
        {mode === 'login' && (
          <div style={{ marginTop: 18, padding: '9px 13px', background: 'rgba(10,10,10,0.45)', border: '1px solid rgba(250,250,250,0.1)', borderRadius: '4px', backdropFilter: 'blur(4px)' }}>
            <div style={{ fontFamily: BODY, fontSize: '10px', color: 'rgba(250,250,250,0.35)', fontWeight: 700, marginBottom: 3, letterSpacing: '0.2px' }}>데모 계정</div>
...
function DiaryTab({ myPosts, onSelectPost, userId, onLogout }: { myPosts: Post[]; onSelectPost: (post: Post) => void; userId: string; onLogout: () => void }) {
...
function MainApp({ currentUser, onLogout }: { currentUser: string; onLogout: () => void }) {
